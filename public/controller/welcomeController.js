...
console.log("in welcomeController");
    ...
    $scope.callAtInterval=  function () {
     for(var j in $rootScope.data_info)
     {
      //  console.log("fsdsf");
       var curr=new Date();
      //  console.log(curr.getDate()+curr.getMonth()+cur);
       var re=$rootScope.data_info[j].remind_at;
      //  console.log(re.);

      var some=new Date(re);
      console.log(curr-some);

       if(curr==some)
       {
         $scope.time();
         console.log("asfjhaksfahiyugasf");
       }
     }
}
...
console.log( $scope.input_div);
      ...
      $rootScope.check=function(){
        ...
        $scope.time = function(){
          alert("jashgdjagsjhas");
                   ngNotify.set('remind alert', {
                       duration: 3000
                   });
               };
...
$scope.pinup=function(id,pin,archive)
{
  ...
  obj.then(function(data) {
    ...
    $localStorage.view = "grid";
      // $scope.grid_v=true;
      // $scope.list_v=false;
      console.log($rootScope.data_info,"hgjhghj");
